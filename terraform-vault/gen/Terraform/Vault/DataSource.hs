...
module Terraform.Vault.DataSource where
...
--
-- Reads AWS credentials from an AWS secret backend in Vault. ~> Important All data retrieved from Vault will be written in cleartext to state file generated by Terraform, will appear in the console output when Terraform runs, and may be included in plan files if secrets are interpolated into any resource attributes. Protect these artifacts accordingly. See <../index.html> for more details.
data Aws_Access_Credentials_DataSource = Aws_Access_Credentials_DataSource
    { backend :: !(Attr Text)
...
type instance Computed Aws_Access_Credentials_DataSource
...
$(TH.makeDataSource
    ...
    ''Vault
    'defaultProvider
    ''Aws_Access_Credentials_DataSource)
...
data Generic_Secret_DataSource = Generic_Secret_DataSource
    { path :: !(Attr Text)
...
type instance Computed Generic_Secret_DataSource
    = '[]
