...
module Terraform.Rundeck.Resource where
...
--
-- The job resource allows Rundeck jobs to be managed by Terraform. In Rundeck a job is a particular named set of steps that can be executed against one or more of the nodes configured for its associated project. Each job belongs to a project. A project can be created with the @rundeck_project@ resource.
data Job_Resource = Job_Resource
    { allow_concurrent_executions :: !(Attr Text)
    ...
    , command_ordering_strategy :: !(Attr Text)
    ...
    , node_filter_exclude_precedence :: !(Attr Text)
    ...
    , preserve_options_order :: !(Attr Text)
...
data Project_Resource = Project_Resource
    { default_node_executor_plugin :: !(Attr Text)
    ...
    , default_node_file_copier_plugin :: !(Attr Text)
    ...
    , resource_model_source :: !(Attr Text)
    ...
    , ssh_authentication_type :: !(Attr Text)
    ...
    , ssh_key_storage_path :: !(Attr Text)
...
--
-- The public key resource allows SSH public keys to be stored into Rundeck's key store. The key store is where Rundeck keeps credentials that are needed to access the nodes on which it runs commands. This resource also allows the retrieval of an existing public key from the store, so that it may be used in the configuration of other resources such as @aws_key_pair@ .
data Public_Key_Resource = Public_Key_Resource
    { delete :: !(Attr Text)
...
type instance Computed Public_Key_Resource
    = '[ '("key_material", Attr Text)
