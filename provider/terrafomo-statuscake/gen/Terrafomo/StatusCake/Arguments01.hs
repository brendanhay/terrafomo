...
-- |
-- Module      : Terrafomo.StatusCake.Arguments01
...
instance HasPostRaw a b => HasPostRaw (TF.Resource l p a) b where
...
instance HasRealBrowser a b => HasRealBrowser (TF.Resource l p a) b where
    realBrowser = TF.configuration . realBrowser

class HasStatusCodes a b | a -> b where
    statusCodes :: P.Lens' a b

instance HasStatusCodes a b => HasStatusCodes (TF.Resource l p a) b where
    statusCodes = TF.configuration . statusCodes

class HasTestTags a b | a -> b where
    testTags :: P.Lens' a b

instance HasTestTags a b => HasTestTags (TF.Resource l p a) b where
    testTags = TF.configuration . testTags

class HasTestType a b | a -> b where
    testType :: P.Lens' a b

instance HasTestType a b => HasTestType (TF.Resource l p a) b where
...
instance HasTriggerRate a b => HasTriggerRate (TF.Resource l p a) b where
    triggerRate = TF.configuration . triggerRate

class HasUseJar a b | a -> b where
    useJar :: P.Lens' a b

instance HasUseJar a b => HasUseJar (TF.Resource l p a) b where
    useJar = TF.configuration . useJar

class HasUserAgent a b | a -> b where
    userAgent :: P.Lens' a b

instance HasUserAgent a b => HasUserAgent (TF.Resource l p a) b where
