...
instance HasAccessLevel a b => HasAccessLevel (TF.Resource l p a) b where
...
instance HasCanCreateGroup a b => HasCanCreateGroup (TF.Resource l p a) b where
    canCreateGroup = TF.configuration . canCreateGroup

class HasCanPush a b | a -> b where
    canPush :: P.Lens' a b

instance HasCanPush a b => HasCanPush (TF.Resource l p a) b where
...
instance HasEnableSslVerification a b => HasEnableSslVerification (TF.Resource l p a) b where
...
instance HasLfsEnabled a b => HasLfsEnabled (TF.Resource l p a) b where
    lfsEnabled = TF.configuration . lfsEnabled

class HasMergeRequestsEnabled a b | a -> b where
    mergeRequestsEnabled :: P.Lens' a b

instance HasMergeRequestsEnabled a b => HasMergeRequestsEnabled (TF.Resource l p a) b where
...
instance HasMergeRequestsEvents a b => HasMergeRequestsEvents (TF.Resource l p a) b where
...
instance HasNoteEvents a b => HasNoteEvents (TF.Resource l p a) b where
...
instance HasPipelineEvents a b => HasPipelineEvents (TF.Resource l p a) b where
...
instance HasProjectsLimit a b => HasProjectsLimit (TF.Resource l p a) b where
    projectsLimit = TF.configuration . projectsLimit

class HasPushEvents a b | a -> b where
    pushEvents :: P.Lens' a b

instance HasPushEvents a b => HasPushEvents (TF.Resource l p a) b where
    pushEvents = TF.configuration . pushEvents

class HasRequestAccessEnabled a b | a -> b where
    requestAccessEnabled :: P.Lens' a b

instance HasRequestAccessEnabled a b => HasRequestAccessEnabled (TF.Resource l p a) b where
...
instance HasSkipConfirmation a b => HasSkipConfirmation (TF.Resource l p a) b where
    skipConfirmation = TF.configuration . skipConfirmation

class HasSnippetsEnabled a b | a -> b where
    snippetsEnabled :: P.Lens' a b

instance HasSnippetsEnabled a b => HasSnippetsEnabled (TF.Resource l p a) b where
    snippetsEnabled = TF.configuration . snippetsEnabled

class HasTagPushEvents a b | a -> b where
    tagPushEvents :: P.Lens' a b

instance HasTagPushEvents a b => HasTagPushEvents (TF.Resource l p a) b where
...
instance HasVisibilityLevel a b => HasVisibilityLevel (TF.Resource l p a) b where
    visibilityLevel = TF.configuration . visibilityLevel

class HasWikiEnabled a b | a -> b where
    wikiEnabled :: P.Lens' a b

instance HasWikiEnabled a b => HasWikiEnabled (TF.Resource l p a) b where
    wikiEnabled = TF.configuration . wikiEnabled

class HasWikiPageEvents a b | a -> b where
    wikiPageEvents :: P.Lens' a b

instance HasWikiPageEvents a b => HasWikiPageEvents (TF.Resource l p a) b where
