...
-- |
-- Module      : Terrafomo.OVH.Arguments01
...
instance HasApplicationKey a b => HasApplicationKey (TF.Resource l p a) b where
    applicationKey = TF.configuration . applicationKey

class HasApplicationSecret a b | a -> b where
    applicationSecret :: P.Lens' a b

instance HasApplicationSecret a b => HasApplicationSecret (TF.Resource l p a) b where
    applicationSecret = TF.configuration . applicationSecret

class HasBackup a b | a -> b where
    backup :: P.Lens' a b

instance HasBackup a b => HasBackup (TF.Resource l p a) b where
    backup = TF.configuration . backup

class HasBalance a b | a -> b where
    balance :: P.Lens' a b

instance HasBalance a b => HasBalance (TF.Resource l p a) b where
...
instance HasConsumerKey a b => HasConsumerKey (TF.Resource l p a) b where
...
instance HasDescriptionRegexp a b => HasDescriptionRegexp (TF.Resource l p a) b where
...
instance HasFarmId a b => HasFarmId (TF.Resource l p a) b where
...
instance HasFieldtype a b => HasFieldtype (TF.Resource l p a) b where
...
instance HasFrontendId a b => HasFrontendId (TF.Resource l p a) b where
...
instance HasIpLoadbalancing a b => HasIpLoadbalancing (TF.Resource l p a) b where
    ipLoadbalancing = TF.configuration . ipLoadbalancing

class HasIpv4 a b | a -> b where
    ipv4 :: P.Lens' a b

instance HasIpv4 a b => HasIpv4 (TF.Resource l p a) b where
...
instance HasNegate a b => HasNegate (TF.Resource l p a) b where
...
instance HasOpenstackRc a b => HasOpenstackRc (TF.Resource l p a) b where
...
instance HasProxyProtocolVersion a b => HasProxyProtocolVersion (TF.Resource l p a) b where
...
instance HasRouteId a b => HasRouteId (TF.Resource l p a) b where
...
instance HasStates a b => HasStates (TF.Resource l p a) b where
...
instance HasSubField a b => HasSubField (TF.Resource l p a) b where
...
instance HasUseDefault a b => HasUseDefault (TF.Resource l p a) b where
    useDefault = TF.configuration . useDefault

class HasUseLastToExpire a b | a -> b where
    useLastToExpire :: P.Lens' a b

instance HasUseLastToExpire a b => HasUseLastToExpire (TF.Resource l p a) b where
    useLastToExpire = TF.configuration . useLastToExpire

class HasUseOldest a b | a -> b where
    useOldest :: P.Lens' a b

instance HasUseOldest a b => HasUseOldest (TF.Resource l p a) b where
...
instance HasVrackEligibility a b => HasVrackEligibility (TF.Resource l p a) b where
    vrackEligibility = TF.configuration . vrackEligibility

class HasVrackId a b | a -> b where
    vrackId :: P.Lens' a b

instance HasVrackId a b => HasVrackId (TF.Resource l p a) b where
    vrackId = TF.configuration . vrackId

class HasVrackName a b | a -> b where
    vrackName :: P.Lens' a b

instance HasVrackName a b => HasVrackName (TF.Resource l p a) b where
    vrackName = TF.configuration . vrackName

class HasVrackNetworkId a b | a -> b where
    vrackNetworkId :: P.Lens' a b

instance HasVrackNetworkId a b => HasVrackNetworkId (TF.Resource l p a) b where
