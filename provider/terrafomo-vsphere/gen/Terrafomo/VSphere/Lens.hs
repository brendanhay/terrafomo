...
instance HasAdapterType a s b => HasAdapterType (TF.Source l p a) s b where
...
instance HasCreateDirectories a s b => HasCreateDirectories (TF.Source l p a) s b where
...
instance HasDatacenterId a s b => HasDatacenterId (TF.Source l p a) s b where
...
instance HasDestinationFile a s b => HasDestinationFile (TF.Source l p a) s b where
...
instance HasDistributedVirtualSwitchUuid a s b => HasDistributedVirtualSwitchUuid (TF.Source l p a) s b where
...
instance HasHostSystemId a s b => HasHostSystemId (TF.Source l p a) s b where
    hostSystemId = TF.configuration . hostSystemId

class HasHostSystemIds a s b | a -> s b where
    hostSystemIds :: Lens' a (TF.Attribute s b)

instance HasHostSystemIds a s b => HasHostSystemIds (TF.Source l p a) s b where
...
instance HasLacpApiVersion a s b => HasLacpApiVersion (TF.Source l p a) s b where
    lacpApiVersion = TF.configuration . lacpApiVersion

class HasLicenseKey a s b | a -> s b where
    licenseKey :: Lens' a (TF.Attribute s b)

instance HasLicenseKey a s b => HasLicenseKey (TF.Source l p a) s b where
...
instance HasLinkDiscoveryOperation a s b => HasLinkDiscoveryOperation (TF.Source l p a) s b where
...
instance HasLinkDiscoveryProtocol a s b => HasLinkDiscoveryProtocol (TF.Source l p a) s b where
...
instance HasManagedObjectType a s b => HasManagedObjectType (TF.Source l p a) s b where
    managedObjectType = TF.configuration . managedObjectType

class HasMaxMtu a s b | a -> s b where
    maxMtu :: Lens' a (TF.Attribute s b)

instance HasMaxMtu a s b => HasMaxMtu (TF.Source l p a) s b where
...
instance HasMtu a s b => HasMtu (TF.Source l p a) s b where
    mtu = TF.configuration . mtu

class HasMulticastFilteringMode a s b | a -> s b where
    multicastFilteringMode :: Lens' a (TF.Attribute s b)

instance HasMulticastFilteringMode a s b => HasMulticastFilteringMode (TF.Source l p a) s b where
...
instance HasNumberOfPorts a s b => HasNumberOfPorts (TF.Source l p a) s b where
...
instance HasScsiControllerScanCount a s b => HasScsiControllerScanCount (TF.Source l p a) s b where
...
instance HasSecurityType a s b => HasSecurityType (TF.Source l p a) s b where
...
instance HasSourceDatacenter a s b => HasSourceDatacenter (TF.Source l p a) s b where
...
instance HasSourceDatastore a s b => HasSourceDatastore (TF.Source l p a) s b where
    sourceDatastore = TF.configuration . sourceDatastore

class HasSourceFile a s b | a -> s b where
    sourceFile :: Lens' a (TF.Attribute s b)

instance HasSourceFile a s b => HasSourceFile (TF.Source l p a) s b where
...
instance HasVirtualMachineUuid a s b => HasVirtualMachineUuid (TF.Source l p a) s b where
...
instance HasVirtualSwitchName a s b => HasVirtualSwitchName (TF.Source l p a) s b where
...
instance HasVmdkPath a s b => HasVmdkPath (TF.Source l p a) s b where
