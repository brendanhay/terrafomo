...
instance HasByteLength a b => HasByteLength (TF.Resource l p a) b where
...
instance HasKeepers a b => HasKeepers (TF.Resource l p a) b where
    keepers = TF.configuration . keepers

class HasLength a b | a -> b where
    length :: P.Lens' a b

instance HasLength a b => HasLength (TF.Resource l p a) b where
    length = TF.configuration . length

class HasLower a b | a -> b where
    lower :: P.Lens' a b

instance HasLower a b => HasLower (TF.Resource l p a) b where
...
instance HasMinLower a b => HasMinLower (TF.Resource l p a) b where
    minLower = TF.configuration . minLower

class HasMinNumeric a b | a -> b where
    minNumeric :: P.Lens' a b

instance HasMinNumeric a b => HasMinNumeric (TF.Resource l p a) b where
    minNumeric = TF.configuration . minNumeric

class HasMinSpecial a b | a -> b where
    minSpecial :: P.Lens' a b

instance HasMinSpecial a b => HasMinSpecial (TF.Resource l p a) b where
    minSpecial = TF.configuration . minSpecial

class HasMinUpper a b | a -> b where
    minUpper :: P.Lens' a b

instance HasMinUpper a b => HasMinUpper (TF.Resource l p a) b where
...
instance HasOverrideSpecial a b => HasOverrideSpecial (TF.Resource l p a) b where
...
instance HasResultCount a b => HasResultCount (TF.Resource l p a) b where
    resultCount = TF.configuration . resultCount

class HasSeed a b | a -> b where
    seed :: P.Lens' a b

instance HasSeed a b => HasSeed (TF.Resource l p a) b where
    seed = TF.configuration . seed

class HasSeparator a b | a -> b where
    separator :: P.Lens' a b

instance HasSeparator a b => HasSeparator (TF.Resource l p a) b where
    separator = TF.configuration . separator

class HasSpecial a b | a -> b where
    special :: P.Lens' a b

instance HasSpecial a b => HasSpecial (TF.Resource l p a) b where
    special = TF.configuration . special

class HasUpper a b | a -> b where
    upper :: P.Lens' a b

instance HasUpper a b => HasUpper (TF.Resource l p a) b where
