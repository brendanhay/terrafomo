...
instance HasAccessModes a b => HasAccessModes (TF.Resource l p a) b where
    accessModes = TF.configuration . accessModes

class HasActiveDeadlineSeconds a b | a -> b where
    activeDeadlineSeconds :: P.Lens' a b

instance HasActiveDeadlineSeconds a b => HasActiveDeadlineSeconds (TF.Resource l p a) b where
...
instance HasAnnotations a b => HasAnnotations (TF.Resource l p a) b where
    annotations = TF.configuration . annotations

class HasApiVersion a b | a -> b where
    apiVersion :: P.Lens' a b

instance HasApiVersion a b => HasApiVersion (TF.Resource l p a) b where
...
instance HasCephFs a b => HasCephFs (TF.Resource l p a) b where
    cephFs = TF.configuration . cephFs

class HasCephMonitors a b | a -> b where
    cephMonitors :: P.Lens' a b

instance HasCephMonitors a b => HasCephMonitors (TF.Resource l p a) b where
    cephMonitors = TF.configuration . cephMonitors

class HasCinder a b | a -> b where
    cinder :: P.Lens' a b

instance HasCinder a b => HasCinder (TF.Resource l p a) b where
    cinder = TF.configuration . cinder

class HasClaimName a b | a -> b where
    claimName :: P.Lens' a b

instance HasClaimName a b => HasClaimName (TF.Resource l p a) b where
...
instance HasClusterCaCertificate a b => HasClusterCaCertificate (TF.Resource l p a) b where
    clusterCaCertificate = TF.configuration . clusterCaCertificate

class HasClusterIp a b | a -> b where
    clusterIp :: P.Lens' a b

instance HasClusterIp a b => HasClusterIp (TF.Resource l p a) b where
...
instance HasConfigContext a b => HasConfigContext (TF.Resource l p a) b where
    configContext = TF.configuration . configContext

class HasConfigContextAuthInfo a b | a -> b where
    configContextAuthInfo :: P.Lens' a b

instance HasConfigContextAuthInfo a b => HasConfigContextAuthInfo (TF.Resource l p a) b where
...
instance HasDataDiskUri a b => HasDataDiskUri (TF.Resource l p a) b where
    dataDiskUri = TF.configuration . dataDiskUri

class HasDatasetName a b | a -> b where
    datasetName :: P.Lens' a b

instance HasDatasetName a b => HasDatasetName (TF.Resource l p a) b where
    datasetName = TF.configuration . datasetName

class HasDatasetUuid a b | a -> b where
    datasetUuid :: P.Lens' a b

instance HasDatasetUuid a b => HasDatasetUuid (TF.Resource l p a) b where
...
instance HasDefaultMode a b => HasDefaultMode (TF.Resource l p a) b where
    defaultMode = TF.configuration . defaultMode

class HasDefaultRequest a b | a -> b where
    defaultRequest :: P.Lens' a b

instance HasDefaultRequest a b => HasDefaultRequest (TF.Resource l p a) b where
    defaultRequest = TF.configuration . defaultRequest

class HasDirectory a b | a -> b where
    directory :: P.Lens' a b

instance HasDirectory a b => HasDirectory (TF.Resource l p a) b where
...
instance HasHostIp a b => HasHostIp (TF.Resource l p a) b where
    hostIp = TF.configuration . hostIp

class HasHostIpc a b | a -> b where
    hostIpc :: P.Lens' a b

instance HasHostIpc a b => HasHostIpc (TF.Resource l p a) b where
    hostIpc = TF.configuration . hostIpc

class HasHostNetwork a b | a -> b where
    hostNetwork :: P.Lens' a b

instance HasHostNetwork a b => HasHostNetwork (TF.Resource l p a) b where
...
instance HasHostPid a b => HasHostPid (TF.Resource l p a) b where
    hostPid = TF.configuration . hostPid

class HasHostPort a b | a -> b where
    hostPort :: P.Lens' a b

instance HasHostPort a b => HasHostPort (TF.Resource l p a) b where
...
instance HasHttpGet a b => HasHttpGet (TF.Resource l p a) b where
    httpGet = TF.configuration . httpGet

class HasHttpHeader a b | a -> b where
    httpHeader :: P.Lens' a b

instance HasHttpHeader a b => HasHttpHeader (TF.Resource l p a) b where
...
instance HasIqn a b => HasIqn (TF.Resource l p a) b where
    iqn = TF.configuration . iqn

class HasIscsi a b | a -> b where
    iscsi :: P.Lens' a b

instance HasIscsi a b => HasIscsi (TF.Resource l p a) b where
    iscsi = TF.configuration . iscsi

class HasIscsiInterface a b | a -> b where
    iscsiInterface :: P.Lens' a b

instance HasIscsiInterface a b => HasIscsiInterface (TF.Resource l p a) b where
    iscsiInterface = TF.configuration . iscsiInterface

class HasItems a b | a -> b where
    items :: P.Lens' a b

instance HasItems a b => HasItems (TF.Resource l p a) b where
...
instance HasLifecycle a b => HasLifecycle (TF.Resource l p a) b where
...
instance HasLimits a b => HasLimits (TF.Resource l p a) b where
    limits = TF.configuration . limits

class HasLivenessProbe a b | a -> b where
    livenessProbe :: P.Lens' a b

instance HasLivenessProbe a b => HasLivenessProbe (TF.Resource l p a) b where
    livenessProbe = TF.configuration . livenessProbe

class HasLoadBalancerIp a b | a -> b where
    loadBalancerIp :: P.Lens' a b

instance HasLoadBalancerIp a b => HasLoadBalancerIp (TF.Resource l p a) b where
...
instance HasLoadBalancerSourceRanges a b => HasLoadBalancerSourceRanges (TF.Resource l p a) b where
...
instance HasLoadConfigFile a b => HasLoadConfigFile (TF.Resource l p a) b where
...
instance HasMatchExpressions a b => HasMatchExpressions (TF.Resource l p a) b where
    matchExpressions = TF.configuration . matchExpressions

class HasMatchLabels a b | a -> b where
    matchLabels :: P.Lens' a b

instance HasMatchLabels a b => HasMatchLabels (TF.Resource l p a) b where
...
instance HasMaxLimitRequestRatio a b => HasMaxLimitRequestRatio (TF.Resource l p a) b where
...
instance HasMedium a b => HasMedium (TF.Resource l p a) b where
...
instance HasMinReadySeconds a b => HasMinReadySeconds (TF.Resource l p a) b where
...
instance HasMonitors a b => HasMonitors (TF.Resource l p a) b where
...
instance HasNfs a b => HasNfs (TF.Resource l p a) b where
    nfs = TF.configuration . nfs

class HasNodeName a b | a -> b where
    nodeName :: P.Lens' a b

instance HasNodeName a b => HasNodeName (TF.Resource l p a) b where
    nodeName = TF.configuration . nodeName

class HasNodePort a b | a -> b where
    nodePort :: P.Lens' a b

instance HasNodePort a b => HasNodePort (TF.Resource l p a) b where
    nodePort = TF.configuration . nodePort

class HasNodeSelector a b | a -> b where
    nodeSelector :: P.Lens' a b

instance HasNodeSelector a b => HasNodeSelector (TF.Resource l p a) b where
...
instance HasOptional a b => HasOptional (TF.Resource l p a) b where
...
instance HasPdId a b => HasPdId (TF.Resource l p a) b where
    pdId = TF.configuration . pdId

class HasPdName a b | a -> b where
    pdName :: P.Lens' a b

instance HasPdName a b => HasPdName (TF.Resource l p a) b where
    pdName = TF.configuration . pdName

class HasPeriodSeconds a b | a -> b where
    periodSeconds :: P.Lens' a b

instance HasPeriodSeconds a b => HasPeriodSeconds (TF.Resource l p a) b where
    periodSeconds = TF.configuration . periodSeconds

class HasPersistentVolumeClaim a b | a -> b where
    persistentVolumeClaim :: P.Lens' a b

instance HasPersistentVolumeClaim a b => HasPersistentVolumeClaim (TF.Resource l p a) b where
...
instance HasPersistentVolumeReclaimPolicy a b => HasPersistentVolumeReclaimPolicy (TF.Resource l p a) b where
...
instance HasPersistentVolumeSource a b => HasPersistentVolumeSource (TF.Resource l p a) b where
...
instance HasPhotonPersistentDisk a b => HasPhotonPersistentDisk (TF.Resource l p a) b where
...
instance HasPostStart a b => HasPostStart (TF.Resource l p a) b where
    postStart = TF.configuration . postStart

class HasPreStop a b | a -> b where
    preStop :: P.Lens' a b

instance HasPreStop a b => HasPreStop (TF.Resource l p a) b where
    preStop = TF.configuration . preStop

class HasPrivileged a b | a -> b where
    privileged :: P.Lens' a b

instance HasPrivileged a b => HasPrivileged (TF.Resource l p a) b where
...
instance HasQuobyte a b => HasQuobyte (TF.Resource l p a) b where
    quobyte = TF.configuration . quobyte

class HasRadosUser a b | a -> b where
    radosUser :: P.Lens' a b

instance HasRadosUser a b => HasRadosUser (TF.Resource l p a) b where
    radosUser = TF.configuration . radosUser

class HasRbd a b | a -> b where
    rbd :: P.Lens' a b

instance HasRbd a b => HasRbd (TF.Resource l p a) b where
    rbd = TF.configuration . rbd

class HasRbdImage a b | a -> b where
    rbdImage :: P.Lens' a b

instance HasRbdImage a b => HasRbdImage (TF.Resource l p a) b where
    rbdImage = TF.configuration . rbdImage

class HasRbdPool a b | a -> b where
    rbdPool :: P.Lens' a b

instance HasRbdPool a b => HasRbdPool (TF.Resource l p a) b where
...
instance HasReadOnlyRootFilesystem a b => HasReadOnlyRootFilesystem (TF.Resource l p a) b where
...
instance HasReadinessProbe a b => HasReadinessProbe (TF.Resource l p a) b where
    readinessProbe = TF.configuration . readinessProbe

class HasRegistry a b | a -> b where
    registry :: P.Lens' a b

instance HasRegistry a b => HasRegistry (TF.Resource l p a) b where
    registry = TF.configuration . registry

class HasReplicas a b | a -> b where
    replicas :: P.Lens' a b

instance HasReplicas a b => HasReplicas (TF.Resource l p a) b where
...
instance HasRequests a b => HasRequests (TF.Resource l p a) b where
...
instance HasResourceFieldRef a b => HasResourceFieldRef (TF.Resource l p a) b where
...
instance HasRunAsNonRoot a b => HasRunAsNonRoot (TF.Resource l p a) b where
    runAsNonRoot = TF.configuration . runAsNonRoot

class HasRunAsUser a b | a -> b where
    runAsUser :: P.Lens' a b

instance HasRunAsUser a b => HasRunAsUser (TF.Resource l p a) b where
    runAsUser = TF.configuration . runAsUser

class HasScaleTargetRef a b | a -> b where
    scaleTargetRef :: P.Lens' a b

instance HasScaleTargetRef a b => HasScaleTargetRef (TF.Resource l p a) b where
...
instance HasSeLinuxOptions a b => HasSeLinuxOptions (TF.Resource l p a) b where
...
instance HasServiceAccountName a b => HasServiceAccountName (TF.Resource l p a) b where
...
instance HasTargetCpuUtilizationPercentage a b => HasTargetCpuUtilizationPercentage (TF.Resource l p a) b where
...
instance HasTerminationGracePeriodSeconds a b => HasTerminationGracePeriodSeconds (TF.Resource l p a) b where
...
instance HasTerminationMessagePath a b => HasTerminationMessagePath (TF.Resource l p a) b where
...
instance HasTimeoutSeconds a b => HasTimeoutSeconds (TF.Resource l p a) b where
...
instance HasTty a b => HasTty (TF.Resource l p a) b where
...
instance HasValueFrom a b => HasValueFrom (TF.Resource l p a) b where
...
instance HasVolumeMount a b => HasVolumeMount (TF.Resource l p a) b where
...
instance HasVolumePath a b => HasVolumePath (TF.Resource l p a) b where
    volumePath = TF.configuration . volumePath

class HasVsphereVolume a b | a -> b where
    vsphereVolume :: P.Lens' a b

instance HasVsphereVolume a b => HasVsphereVolume (TF.Resource l p a) b where
    vsphereVolume = TF.configuration . vsphereVolume

class HasWaitUntilBound a b | a -> b where
    waitUntilBound :: P.Lens' a b

instance HasWaitUntilBound a b => HasWaitUntilBound (TF.Resource l p a) b where
