...
instance HasComposite a s b => HasComposite (TF.Source l p a) s b where
...
instance HasMax a s b => HasMax (TF.Source l p a) s b where
    max = TF.configuration . max

class HasMin a s b | a -> s b where
    min :: Lens' a (TF.Attribute s b)

instance HasMin a s b => HasMin (TF.Source l p a) s b where
...
instance HasRearmSeconds a s b => HasRearmSeconds (TF.Source l p a) s b where
    rearmSeconds = TF.configuration . rearmSeconds

class HasRelatedSpace a s b | a -> s b where
    relatedSpace :: Lens' a (TF.Attribute s b)

instance HasRelatedSpace a s b => HasRelatedSpace (TF.Source l p a) s b where
...
instance HasSpaceId a s b => HasSpaceId (TF.Source l p a) s b where
    spaceId = TF.configuration . spaceId

class HasStream a s b | a -> s b where
    stream :: Lens' a (TF.Attribute s b)

instance HasStream a s b => HasStream (TF.Source l p a) s b where
