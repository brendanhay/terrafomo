...
instance TF.IsValid (Nic s) where
    validator = P.mempty
           P.<> TF.settingsValidator "_firewall"
                  (_firewall
                      :: Nic s -> TF.Attr s [TF.Attr s (Firewall s)])
                  TF.validator
