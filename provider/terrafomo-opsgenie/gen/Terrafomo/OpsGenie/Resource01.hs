...
data TeamResource s = TeamResource'
    ...
    --
    , _member      :: TF.Attr s [TF.Attr s (TeamMemberSetting s)]
    -- ^ @member@ - (Optional)
    --
...
instance P.HasMember (TeamResource s) (TF.Attr s [TF.Attr s (TeamMemberSetting s)]) where
    member =
        P.lens (_member :: TeamResource s -> TF.Attr s [TF.Attr s (TeamMemberSetting s)])
