...
instance TF.IsValid (AppServiceData s) where
    validator = P.mempty
           P.<> TF.settingsValidator "_siteConfig"
                  (_siteConfig
                      :: AppServiceData s -> TF.Attr s (AppServiceSiteConfig s))
                  TF.validator
...
instance P.HasSiteConfig (AppServiceData s) (TF.Attr s (AppServiceSiteConfig s)) where
    siteConfig =
        P.lens (_siteConfig :: AppServiceData s -> TF.Attr s (AppServiceSiteConfig s))
...
storageAccountSasData
    ...
    -> TF.Attr s (StorageAccountSasPermissions s) -- ^ @permissions@ ('P._permissions', 'P.permissions')
    -> TF.Attr s (StorageAccountSasServices s) -- ^ @services@ ('P._services', 'P.services')
    -> TF.Attr s P.Text -- ^ @start@ ('P._start', 'P.start')
    -> TF.Attr s P.Text -- ^ @connection_string@ ('P._connectionString', 'P.connectionString')
    -> TF.Attr s (StorageAccountSasResourceTypes s) -- ^ @resource_types@ ('P._resourceTypes', 'P.resourceTypes')
