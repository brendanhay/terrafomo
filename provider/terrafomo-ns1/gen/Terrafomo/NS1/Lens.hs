...
instance HasAnswers a s b => HasAnswers (TF.Source l p a) s b where
...
instance HasExpiry a s b => HasExpiry (TF.Source l p a) s b where
    expiry = TF.configuration . expiry

class HasFilters a s b | a -> s b where
    filters :: Lens' a (TF.Attribute s b)

instance HasFilters a s b => HasFilters (TF.Source l p a) s b where
...
instance HasJobType a s b => HasJobType (TF.Source l p a) s b where
...
instance HasNotifyRepeat a s b => HasNotifyRepeat (TF.Source l p a) s b where
    notifyRepeat = TF.configuration . notifyRepeat

class HasNxTtl a s b | a -> s b where
    nxTtl :: Lens' a (TF.Attribute s b)

instance HasNxTtl a s b => HasNxTtl (TF.Source l p a) s b where
...
instance HasRetry a s b => HasRetry (TF.Source l p a) s b where
...
instance HasSourceId a s b => HasSourceId (TF.Source l p a) s b where
...
instance HasUseClientSubnet a s b => HasUseClientSubnet (TF.Source l p a) s b where
