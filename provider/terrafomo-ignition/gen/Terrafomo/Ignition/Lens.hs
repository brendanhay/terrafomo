...
instance HasAppend a s b => HasAppend (TF.Source l p a) s b where
    append = TF.configuration . append

class HasArrays a s b | a -> s b where
    arrays :: Lens' a (TF.Attribute s b)

instance HasArrays a s b => HasArrays (TF.Source l p a) s b where
...
instance HasDevices a s b => HasDevices (TF.Source l p a) s b where
    devices = TF.configuration . devices

class HasDirectories a s b | a -> s b where
    directories :: Lens' a (TF.Attribute s b)

instance HasDirectories a s b => HasDirectories (TF.Source l p a) s b where
...
instance HasDropin a s b => HasDropin (TF.Source l p a) s b where
...
instance HasHard a s b => HasHard (TF.Source l p a) s b where
    hard = TF.configuration . hard

class HasHomeDir a s b | a -> s b where
    homeDir :: Lens' a (TF.Attribute s b)

instance HasHomeDir a s b => HasHomeDir (TF.Source l p a) s b where
...
instance HasUid a s b => HasUid (TF.Source l p a) s b where
...
instance HasWipeTable a s b => HasWipeTable (TF.Source l p a) s b where
