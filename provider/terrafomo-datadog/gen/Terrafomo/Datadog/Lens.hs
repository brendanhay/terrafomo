...
instance HasEscalationMessage a s b => HasEscalationMessage (TF.Source l p a) s b where
...
instance HasLocked a s b => HasLocked (TF.Source l p a) s b where
    locked = TF.configuration . locked

class HasMessage a s b | a -> s b where
    message :: Lens' a (TF.Attribute s b)

instance HasMessage a s b => HasMessage (TF.Source l p a) s b where
...
instance HasMonitorId a s b => HasMonitorId (TF.Source l p a) s b where
...
instance HasNewHostDelay a s b => HasNewHostDelay (TF.Source l p a) s b where
...
instance HasNoDataTimeframe a s b => HasNoDataTimeframe (TF.Source l p a) s b where
...
instance HasRenotifyInterval a s b => HasRenotifyInterval (TF.Source l p a) s b where
...
instance HasRequireFullWindow a s b => HasRequireFullWindow (TF.Source l p a) s b where
...
instance HasSilenced a s b => HasSilenced (TF.Source l p a) s b where
...
instance HasStatsdInterval a s b => HasStatsdInterval (TF.Source l p a) s b where
...
instance HasTemplateVariable a s b => HasTemplateVariable (TF.Source l p a) s b where
...
instance HasThresholds a s b => HasThresholds (TF.Source l p a) s b where
    thresholds = TF.configuration . thresholds

class HasTimeoutH a s b | a -> s b where
    timeoutH :: Lens' a (TF.Attribute s b)

instance HasTimeoutH a s b => HasTimeoutH (TF.Source l p a) s b where
