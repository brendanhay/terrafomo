...
instance HasCatalogName a s b => HasCatalogName (TF.Source l p a) s b where
    catalogName = TF.configuration . catalogName

class HasCpus a s b | a -> s b where
    cpus :: Lens' a (TF.Attribute s b)

instance HasCpus a s b => HasCpus (TF.Source l p a) s b where
...
instance HasEncryptionProtocol a s b => HasEncryptionProtocol (TF.Source l p a) s b where
...
instance HasInitscript a s b => HasInitscript (TF.Source l p a) s b where
...
instance HasLocalId a s b => HasLocalId (TF.Source l p a) s b where
    localId = TF.configuration . localId

class HasLocalIpAddress a s b | a -> s b where
    localIpAddress :: Lens' a (TF.Attribute s b)

instance HasLocalIpAddress a s b => HasLocalIpAddress (TF.Source l p a) s b where
...
instance HasLocalSubnets a s b => HasLocalSubnets (TF.Source l p a) s b where
...
instance HasNetworkName a s b => HasNetworkName (TF.Source l p a) s b where
    networkName = TF.configuration . networkName

class HasOvf a s b | a -> s b where
    ovf :: Lens' a (TF.Attribute s b)

instance HasOvf a s b => HasOvf (TF.Source l p a) s b where
    ovf = TF.configuration . ovf

class HasPeerId a s b | a -> s b where
    peerId :: Lens' a (TF.Attribute s b)

instance HasPeerId a s b => HasPeerId (TF.Source l p a) s b where
...
instance HasPeerSubnets a s b => HasPeerSubnets (TF.Source l p a) s b where
...
instance HasPowerOn a s b => HasPowerOn (TF.Source l p a) s b where
...
instance HasStaticIpPool a s b => HasStaticIpPool (TF.Source l p a) s b where
...
instance HasVappName a s b => HasVappName (TF.Source l p a) s b where
