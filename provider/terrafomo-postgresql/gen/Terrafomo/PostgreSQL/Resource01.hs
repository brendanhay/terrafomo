...
data SchemaResource s = SchemaResource'
    ...
    --
    , _policy      :: TF.Attr s [TF.Attr s (SchemaPolicy s)]
    -- ^ @policy@ - (Optional)
    --
    } deriving (P.Show, P.Eq, P.Ord)
...
instance P.HasPolicy (SchemaResource s) (TF.Attr s [TF.Attr s (SchemaPolicy s)]) where
    policy =
        P.lens (_policy :: SchemaResource s -> TF.Attr s [TF.Attr s (SchemaPolicy s)])
