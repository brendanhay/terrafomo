...
instance HasAppliesTo a s b => HasAppliesTo (TF.Source l p a) s b where
...
instance HasDisableAlerting a s b => HasDisableAlerting (TF.Source l p a) s b where
...
instance HasHostgroupId a s b => HasHostgroupId (TF.Source l p a) s b where
    hostgroupId = TF.configuration . hostgroupId

class HasIpAddr a s b | a -> s b where
    ipAddr :: Lens' a (TF.Attribute s b)

instance HasIpAddr a s b => HasIpAddr (TF.Source l p a) s b where
...
instance HasOffset a s b => HasOffset (TF.Source l p a) s b where
