...
-- |
-- Module      : Terrafomo.OneAndOne.Arguments01
...
instance HasAgent a b => HasAgent (TF.Resource l p a) b where
    agent = TF.configuration . agent

class HasAlert a b | a -> b where
    alert :: P.Lens' a b

instance HasAlert a b => HasAlert (TF.Resource l p a) b where
    alert = TF.configuration . alert

class HasAlertIf a b | a -> b where
    alertIf :: P.Lens' a b

instance HasAlertIf a b => HasAlertIf (TF.Resource l p a) b where
    alertIf = TF.configuration . alertIf

class HasCoresPerProcessor a b | a -> b where
    coresPerProcessor :: P.Lens' a b

instance HasCoresPerProcessor a b => HasCoresPerProcessor (TF.Resource l p a) b where
...
instance HasEmailNotification a b => HasEmailNotification (TF.Resource l p a) b where
...
instance HasFirewallPolicyId a b => HasFirewallPolicyId (TF.Resource l p a) b where
    firewallPolicyId = TF.configuration . firewallPolicyId

class HasFixedInstanceSize a b | a -> b where
    fixedInstanceSize :: P.Lens' a b

instance HasFixedInstanceSize a b => HasFixedInstanceSize (TF.Resource l p a) b where
...
instance HasHdds a b => HasHdds (TF.Resource l p a) b where
...
instance HasHealthCheckPath a b => HasHealthCheckPath (TF.Resource l p a) b where
...
instance HasHealthCheckPathParser a b => HasHealthCheckPathParser (TF.Resource l p a) b where
...
instance HasHealthCheckTest a b => HasHealthCheckTest (TF.Resource l p a) b where
...
instance HasInternalPing a b => HasInternalPing (TF.Resource l p a) b where
...
instance HasIpType a b => HasIpType (TF.Resource l p a) b where
    ipType = TF.configuration . ipType

class HasIsMain a b | a -> b where
    isMain :: P.Lens' a b

instance HasIsMain a b => HasIsMain (TF.Resource l p a) b where
...
instance HasMonitoringPolicyId a b => HasMonitoringPolicyId (TF.Resource l p a) b where
...
instance HasNetworkAddress a b => HasNetworkAddress (TF.Resource l p a) b where
    networkAddress = TF.configuration . networkAddress

class HasNumImages a b | a -> b where
    numImages :: P.Lens' a b

instance HasNumImages a b => HasNumImages (TF.Resource l p a) b where
    numImages = TF.configuration . numImages

class HasOsId a b | a -> b where
    osId :: P.Lens' a b

instance HasOsId a b => HasOsId (TF.Resource l p a) b where
...
instance HasPersistenceTime a b => HasPersistenceTime (TF.Resource l p a) b where
...
instance HasPortBalancer a b => HasPortBalancer (TF.Resource l p a) b where
    portBalancer = TF.configuration . portBalancer

class HasPortFrom a b | a -> b where
    portFrom :: P.Lens' a b

instance HasPortFrom a b => HasPortFrom (TF.Resource l p a) b where
    portFrom = TF.configuration . portFrom

class HasPortServer a b | a -> b where
    portServer :: P.Lens' a b

instance HasPortServer a b => HasPortServer (TF.Resource l p a) b where
    portServer = TF.configuration . portServer

class HasPortTo a b | a -> b where
    portTo :: P.Lens' a b

instance HasPortTo a b => HasPortTo (TF.Resource l p a) b where
...
instance HasProcesses a b => HasProcesses (TF.Resource l p a) b where
...
instance HasRights a b => HasRights (TF.Resource l p a) b where
...
instance HasSshKeyPublic a b => HasSshKeyPublic (TF.Resource l p a) b where
    sshKeyPublic = TF.configuration . sshKeyPublic

class HasStorageServers a b | a -> b where
    storageServers :: P.Lens' a b

instance HasStorageServers a b => HasStorageServers (TF.Resource l p a) b where
...
instance HasTransfer a b => HasTransfer (TF.Resource l p a) b where
...
instance HasVcores a b => HasVcores (TF.Resource l p a) b where
