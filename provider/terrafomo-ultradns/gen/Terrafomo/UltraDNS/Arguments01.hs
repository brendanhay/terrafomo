...
-- |
-- Module      : Terrafomo.UltraDNS.Arguments01
...
instance HasActOnProbes a b => HasActOnProbes (TF.Resource l p a) b where
...
instance HasAgents a b => HasAgents (TF.Resource l p a) b where
    agents = TF.configuration . agents

class HasAllNonConfigured a b | a -> b where
    allNonConfigured :: P.Lens' a b

instance HasAllNonConfigured a b => HasAllNonConfigured (TF.Resource l p a) b where
...
instance HasBackupRecordFailoverDelay a b => HasBackupRecordFailoverDelay (TF.Resource l p a) b where
...
instance HasBackupRecordRdata a b => HasBackupRecordRdata (TF.Resource l p a) b where
...
instance HasCodes a b => HasCodes (TF.Resource l p a) b where
    codes = TF.configuration . codes

class HasConflictResolve a b | a -> b where
    conflictResolve :: P.Lens' a b

instance HasConflictResolve a b => HasConflictResolve (TF.Resource l p a) b where
...
instance HasHttpProbe a b => HasHttpProbe (TF.Resource l p a) b where
...
instance HasIpInfo a b => HasIpInfo (TF.Resource l p a) b where
...
instance HasIsAccountLevel a b => HasIsAccountLevel (TF.Resource l p a) b where
...
instance HasMaxToLb a b => HasMaxToLb (TF.Resource l p a) b where
...
instance HasNoResponse a b => HasNoResponse (TF.Resource l p a) b where
...
instance HasPacketSize a b => HasPacketSize (TF.Resource l p a) b where
    packetSize = TF.configuration . packetSize

class HasPackets a b | a -> b where
    packets :: P.Lens' a b

instance HasPackets a b => HasPackets (TF.Resource l p a) b where
...
instance HasPingProbe a b => HasPingProbe (TF.Resource l p a) b where
    pingProbe = TF.configuration . pingProbe

class HasPoolRecord a b | a -> b where
    poolRecord :: P.Lens' a b

instance HasPoolRecord a b => HasPoolRecord (TF.Resource l p a) b where
...
instance HasRdata a b => HasRdata (TF.Resource l p a) b where
    rdata = TF.configuration . rdata

class HasRunProbes a b | a -> b where
    runProbes :: P.Lens' a b

instance HasRunProbes a b => HasRunProbes (TF.Resource l p a) b where
...
instance HasTotalLimits a b => HasTotalLimits (TF.Resource l p a) b where
    totalLimits = TF.configuration . totalLimits

class HasTransaction a b | a -> b where
    transaction :: P.Lens' a b

instance HasTransaction a b => HasTransaction (TF.Resource l p a) b where
    transaction = TF.configuration . transaction

class HasTransmittedData a b | a -> b where
    transmittedData :: P.Lens' a b

instance HasTransmittedData a b => HasTransmittedData (TF.Resource l p a) b where
