...
data NoResponse s = NoResponse'
    ...
    --
    , _geoInfo          :: TF.Attr s [GeoInfo s]
    -- ^ @geo_info@ - (Optional)
    --
    , _ipInfo           :: TF.Attr s [IpInfo s]
    -- ^ @ip_info@ - (Optional)
    --
    } deriving (P.Show, P.Eq, P.Generic)
...
instance P.HasAllNonConfigured (NoResponse s) (TF.Attr s P.Bool) where
    allNonConfigured =
        P.lens (_allNonConfigured :: NoResponse s -> TF.Attr s P.Bool)
               (\s a -> s { _allNonConfigured = a
                          } :: NoResponse s)

instance P.HasGeoInfo (NoResponse s) (TF.Attr s [GeoInfo s]) where
    geoInfo =
        P.lens (_geoInfo :: NoResponse s -> TF.Attr s [GeoInfo s])
               (\s a -> s { _geoInfo = a
                          } :: NoResponse s)

instance P.HasIpInfo (NoResponse s) (TF.Attr s [IpInfo s]) where
    ipInfo =
        P.lens (_ipInfo :: NoResponse s -> TF.Attr s [IpInfo s])
               (\s a -> s { _ipInfo = a
                          } :: NoResponse s)
...
instance TF.IsObject (Transaction s) where
    toObject Transaction'{..} = catMaybes
...
data GeoInfo s = GeoInfo'
    { _codes          :: TF.Attr s [TF.Attr s (TF.Attr s P.Text)]
    -- ^ @codes@ - (Optional)
    --
    , _isAccountLevel :: TF.Attr s P.Bool
    -- ^ @is_account_level@ - (Optional)
...
instance P.HasCodes (GeoInfo s) (TF.Attr s [TF.Attr s (TF.Attr s P.Text)]) where
    codes =
        P.lens (_codes :: GeoInfo s -> TF.Attr s [TF.Attr s (TF.Attr s P.Text)])
               (\s a -> s { _codes = a
                          } :: GeoInfo s)
...
data HttpProbe s = HttpProbe'
    { _totalLimits :: TF.Attr s [TotalLimits s]
    -- ^ @total_limits@ - (Optional)
    --
    , _transaction :: TF.Attr s [Transaction s]
...
instance P.HasTotalLimits (HttpProbe s) (TF.Attr s [TotalLimits s]) where
    totalLimits =
        P.lens (_totalLimits :: HttpProbe s -> TF.Attr s [TotalLimits s])
               (\s a -> s { _totalLimits = a
                          } :: HttpProbe s)

instance P.HasTransaction (HttpProbe s) (TF.Attr s [Transaction s]) where
    transaction =
        P.lens (_transaction :: HttpProbe s -> TF.Attr s [Transaction s])
               (\s a -> s { _transaction = a
                          } :: HttpProbe s)
...
instance TF.IsObject (TotalLimits s) where
    toObject TotalLimits'{..} = catMaybes
        [ TF.assign "critical" <$> TF.attribute _critical
        , TF.assign "fail" <$> TF.attribute _fail
        , TF.assign "warning" <$> TF.attribute _warning
        ]
